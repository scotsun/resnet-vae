...
def line_from_points(c0, r0, c1, r1):
    ...
    cc, rr = np.meshgrid(np.linspace(0, 31, 32), np.linspace(0, 27, 32), sparse=True)
...
def fog(x, severity=5):
    ...
    x = np.array(x) / 255.0
    max_val = x.max()
    x = x + c[0] * plasma_fractal(wibbledecay=c[1])[:32, :32]
...
def rgb_change(x, color: str):
    ...
    rgb_img = np.zeros((32, 32, 3), dtype=np.float32)
